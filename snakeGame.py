...
def gameLoop():  # Main function for the game
    ...
    while not game_over:
        ...
        if bonus_ball_active:
            pygame.draw.rect(dis, red, [bonus_foodx, bonus_foody, snake_block, snake_block])
            bonus_ball_timer += 1
            if bonus_ball_timer >= bonus_duration * snake_speed:  # Convert seconds to frames
                bonus_ball_active = False
                bonus_ball_timer = 0

        # Check if snake eats bonus ball
        if x1 == bonus_foodx and y1 == bonus_foody:
            Length_of_snake += 1
            bonus_ball_active = False

        # Generate new bonus ball after every 5 regular food eaten
        if Length_of_snake % 5 == 0 and Length_of_snake > 1 and not bonus_ball_active:
